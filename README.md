...
A button-driven Telegram bot for bulk-editing captions and bulk-deleting old posts across channels, groups, and forum topics — built on the **Telegram Bot API** (via [aiogram](https://docs.aiogram.dev/)), backed by **PostgreSQL**.

> 🚀 **This bot can be deployed on Render, Heroku, Koyeb, Railway, Google Cloud Run, Google Colab, VPS, and Termux.** See [Deployment Guide](#-deployment-guide).
...
- [Overview](#-overview)
- [Features](#-features)
  - [Caption Manager](#️-caption-manager)
  - [Post Manager](#-post-manager)
  - [Shared Across Both Modules](#-shared-across-both-modules)
- [Complete UI / Button Guide](#-complete-ui--button-guide)
- [Commands](#-commands)
- [Help Button](#-help-button)
- [Keep Alive](#-keep-alive)
- [Job / Progress UI](#-job--progress-ui)
- [How to Use](#-how-to-use)
- [Requirements / Setup](#️-requirements--setup)
- [Environment Variables](#-environment-variables)
- [Deployment Guide](#-deployment-guide)
- [Tech Stack](#-tech-stack)
- [Important Libraries / APIs](#-important-libraries--apis)
...
| `RUN_MODE` | ❌ No | `webhook` | `webhook` (default — Render/Heroku/Koyeb/Railway/Cloud Run, any platform with a public HTTPS URL) or `polling` (Google Colab, local dev, or anywhere without a public URL — no web server or webhook is set up, updates are pulled directly instead) |
| `WEBHOOK_URL` | ✅ Yes in webhook mode | — | Public base URL of the deployed service (no trailing slash). Not used in `polling` mode |
| `WEBHOOK_SECRET` | ✅ Yes in webhook mode | — | Random secret validating that webhook requests come from Telegram. Not used in `polling` mode |
...
| `WEBHOOK_PATH` | ❌ No | `/webhook` | URL path Telegram POSTs updates to. Not used in `polling` mode |
| `PORT` | ❌ No | `10000` | Port the server binds to (most platforms set this automatically). Not used in `polling` mode |
...
This bot supports **Render, Heroku, Koyeb, Railway, Google Cloud Run, Google Colab, VPS, and Termux**. Render is the most-tested platform; everything else works via the same `Dockerfile`/`Procfile`/`app.json`/polling-mode support included in this repo.

### One-Click Deploy

| Platform | Deploy |
|---|---|
| Render | [![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/cosmos475/post-manger-) |
| Heroku | [![Deploy to Heroku](https://www.herokucdn.com/deploy/button.svg)](https://heroku.com/deploy?template=https://github.com/cosmos475/post-manger-) |
| Koyeb | [![Deploy to Koyeb](https://www.koyeb.com/static/images/deploy/button.svg)](https://app.koyeb.com/deploy?type=git&repository=github.com/cosmos475/post-manger-&branch=main&name=post-manager-bot) |
| Google Colab | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/cosmos475/post-manger-/blob/main/colab_deploy.ipynb) |
| Google Cloud | [![Open in Cloud Shell](https://gstatic.com/cloudssh/images/open-btn.svg)](https://ssh.cloud.google.com/cloudshell/editor?cloudshell_git_repo=https://github.com/cosmos475/post-manger-&cloudshell_tutorial=.cloudshell/GCLOUD.md) |
...
Uses the included `app.json` and `Procfile`. After clicking the badge above, fill in the prompted fields — `WEBHOOK_URL` should be `https://<your-app-name>.herokuapp.com` (update it after the app is created if the auto-filled name doesn't match your final app name), and `WEBHOOK_SECRET` auto-generates.
...
Click the Colab badge above to open `colab_deploy.ipynb`. Fill in the form fields (`BOT_TOKEN`, `OWNER_ID`, `DATABASE_URL`, `SCRATCH_CHAT_ID`) and run the single cell — it clones the repo, installs dependencies, and runs the bot in `RUN_MODE=polling` (hardcoded, since Colab has no stable public URL for webhooks). The cell blocks and streams logs live; press ■ to stop.
...
```bash
git clone https://github.com/cosmos475/post-manger-.git
cd post-manger-
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
cp .env.example .env
# edit .env with your values — set RUN_MODE=webhook if you have a public
# domain/reverse-proxy in front of this, or RUN_MODE=polling otherwise
python3 main.py
```
...
sudo docker build -t post-manager-bot .
sudo docker run -it --rm --env-file .env post-manager-bot
```

Telegram webhooks require a public HTTPS URL — use a tunnel (e.g. `ngrok http 10000`) for local/VPS testing without a domain, and set `WEBHOOK_URL` to the tunnel URL. Alternatively, set `RUN_MODE=polling` to skip webhooks entirely.
...
pip install -r requirements.txt
cp .env.example .env
# edit .env — set RUN_MODE=polling (Termux has no public URL)
python3 main.py
```

> Termux has no package for PostgreSQL's native client libraries pre-installed in all cases — if `asyncpg` fails to build, run `pkg install libpq` first. A remote Postgres instance (e.g. [Neon](https://neon.tech)'s free tier) is recommended over trying to run Postgres on-device.
...
python main.py
```
Telegram webhooks require a public HTTPS URL — use a tunnel (e.g. `ngrok http 10000`) for local testing and set `WEBHOOK_URL` to the tunnel URL, or set `RUN_MODE=polling` to skip webhooks entirely.
...
```text
post-utility--main/
├── main.py                        # Entrypoint: webhook mode (aiohttp app + /health) or polling mode, via RUN_MODE
...
├── colab_deploy.ipynb             # Google Colab one-click deploy notebook (polling mode)
